...
public class UsuarioService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String userName) throws UsernameNotFoundException {
        Usuario usuario = buscarPorEmailAtivo(userName).orElseThrow(
                () -> new UsernameNotFoundException("Usuário " + userName + " não foi encontrado!"));
        return new User(
                usuario.getEmail(),
                usuario.getSenha(),
    ...
    private void emailDeConfirmacaoDeCadastro(String email) throws MessagingException {
        String codigoConfirmacao = Base64Utils.encodeToString(email.getBytes());
        emailService.enviarPedidoDeConfirmacaoDeCadastro(email, codigoConfirmacao);
    }
